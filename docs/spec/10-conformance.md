...
테스트는 내부 함수의 이름이나 파일 분할이 아니라, **스펙이 요구한 결과**를 확인한다. 단, `AEUI.__runtime`처럼 컴파일러와 런타임 사이의 ABI로 스펙에 명시된 값만 예외적으로 직접 확인한다. 12장(소스 파일 배치)이 규정하는 파일 경계와 private 심볼 배치는 **source import를 통해 검증**하며, 이는 이 장의 "내부 함수 이름이나 파일 분할을 검사하지 않는다"는 원칙의 예외다.
