...
디렉터리 라우터는 AEUI core 제품에 포함된 기본 라우팅 방식이다. 선택적 router package를 별도로 설치하지 않는다. `src/pages` 디렉터리에 페이지 파일을 만들면 파일 경로가 그대로 URL이 되고, 화면 이동은 HTML에서 쓰던 것처럼 일반 `<a href="/path">` 링크를 사용한다.
