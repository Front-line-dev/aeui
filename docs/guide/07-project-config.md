...
| 항목 | 설명 |
|---|---|
| **설정 파일** | `vite.config.js`에서 `aeui()` 플러그인 추가 |
| **경로 별칭** | `@ → src/` (기본) |
| **모드** | `src/pages/` 유무에 따라 자동 선택 |
| **HTML** | script 태그 불필요, 자동 주입 |
| **npm 이름** | `aeui` → `npm:a-easy-ui` alias |
| **Babel 순서** | AEUI 플러그인 → JSX 플러그인 |
...
- 내부 빌드 구조: [개발 문서 — Babel 컴파일러](../dev/07-babel-compiler.md)
