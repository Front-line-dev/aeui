...
- `tick`은 `requestAnimationFrame` 루프에서 발생시킨다.
- `tick` 발생시 이미 렌더중인 경우 렌더하지 않는다.
- 렌더 결과에 실제 변화가 있는 경우 다음 프레임에서 다시 렌더한다. (1 프레임 간격)
- 변화가 없으면 간격을 늘린다: `2 → 4 → 8 ...` 최대 `60` 프레임.
- `tick`은 내부 함수 `_tick`으로 수행되며, 변화 여부를 boolean으로 반환한다.
- 유저는 `AEUI.render()`로 수동 렌더를 즉시 호출할 수 있다.
...
- 최초 렌더 1회 실행
- `requestAnimationFrame` 기반 스케줄러 시작
  - 변화가 있을 때는 빠른 간격(1프레임) 유지
  - 변화가 없을 때는 점진적으로 간격 증가 (최대 60프레임)
