...
AEUI에는 `setState`가 없습니다. 대신 일반 `let` 변수가 상태이고, AEUI가 **자동으로 변경을 감지**해서 화면을 갱신합니다. 이 장에서는 이 자동 감지 위에 `watch`와 `clean`을 활용하는 방법을 설명합니다.
...
- `count++`는 그냥 JavaScript 변수 변경입니다.
- AEUI가 주기적으로 값을 확인하고, 바뀌었으면 화면을 갱신합니다.
