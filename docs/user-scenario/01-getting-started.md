...
새 AEUI 프로젝트를 만들고 실행해 봅니다. 페이지에 표시할 내용을 작성한 뒤 브라우저에서 확인합니다.

---

## 프로젝트 만들기

Node.js `^22.12.0 || >=24.0.0`이 필요합니다. 현재 `0.1.0-alpha.1`은 릴리즈 준비 중입니다. npm 발행 전에는 [저장소 실행 방법](../../README.md#지금-실행하기)을 사용하고, 발행 완료 후 아래 명령을 실행합니다.

프로젝트를 만들 디렉터리에서 실행합니다:
...
저장한 뒤 브라우저에서 제목과 본문을 확인합니다. 제목을 다른 문장으로 바꾸고 다시 저장하면 화면에도 반영됩니다. 다음 단계에서는 [컴포넌트에 상태를 추가하고 버튼으로 바꾸는 방법](02-components.md)을 다룹니다.
