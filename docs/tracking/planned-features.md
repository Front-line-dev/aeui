...
이 문서는 아직 현재 AEUI 스펙에 포함하지 않은 기능과 개선 작업을 관리한다. 여기 적힌 내용은 지금 반드시 구현해야 하는 동작이나 테스트 기대값이 아니다. 구현을 시작할 때 입력, 출력, 오류와 예외 조건을 확정해 [`docs/spec`](../spec/README.md)으로 옮긴다.
