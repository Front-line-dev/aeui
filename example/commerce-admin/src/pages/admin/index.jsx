      ...
      <div className="panel__body">
        <div className="grid" style="grid-template-columns: repeat(12, 1fr); gap: 12px;">
          <div style="grid-column: span 6;">
            <StatCard
              kicker="상품"
              value={String(getProductStats(state.products).productCount)}
              title="상품"
              hint={`활성 ${getProductStats(state.products).activeCount} · 비활성 ${getProductStats(state.products).inactiveCount}`}
            />
          </div>
          <div style="grid-column: span 6;">
            <StatCard
              kicker="재고"
              value={`${getProductStats(state.products).outOfStockCount}/${getProductStats(state.products).lowStockCount}`}
              title="품절/저재고(활성)"
          ...
          <div style="grid-column: span 6;">
            <StatCard
              kicker="주문"
              value={String(getOrderStats(state.orders).orderCount)}
              title="주문"
              hint={`PAID ${getOrderStats(state.orders).paidCount} · SHIPPED ${getOrderStats(state.orders).shippedCount} · DELIVERED ${getOrderStats(state.orders).deliveredCount} · CANCELED ${getOrderStats(state.orders).canceledCount}`}
            />
          </div>
          <div style="grid-column: span 6;">
            <StatCard kicker="매출" value={formatKRW(getOrderStats(state.orders).revenue)} title="매출(스냅샷 합)" hint="취소 주문은 제외합니다." />
          </div>
          ...
          <div style="grid-column: span 12;">
            <div className="split">
              ...
              <ActivityFeed items={getRecentActivity(state.activity)} />
            </div>
          </div>
        </div>
      </div>
