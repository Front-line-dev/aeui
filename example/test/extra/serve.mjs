...
if (name === 'standalone') {
  const server = await createServer({ root, configFile: false, server: { host: '127.0.0.1', port, strictPort: true } });
  await server.listen();
} else if (name === 'cli' || name === 'cli-dev') {
  ...
  if (!['commerce-admin', 'vite-demo', 'deep-compare-test'].includes(name)) throw new Error('Unknown reference app');
