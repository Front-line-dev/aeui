...
export default defineConfig({
  ...
  webServer: ['cli', 'commerce-admin', 'vite-demo', 'deep-compare-test', 'standalone', 'cli-dev'].map((name, offset) => ({
    command: `node example/test/extra/serve.mjs ${name} ${port + offset}`,
    cwd: root,
    url: `http://127.0.0.1:${port + offset}${name === 'standalone' ? '/example/letProps/' : '/'}`,
